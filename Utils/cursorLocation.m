...
function cursorLocation(obj, event, location, format_str, text_color)
    ...
    axes_handle_all = findobj(obj, 'Type', 'axes');
    
    if (isempty(axes_handle_all))
        % Only handles up to 2 axes
        return
    end
    % Read relevant axes handle
    for i = 1: numel(axes_handle_all)
        axes_handle = axes_handle_all(i);       
        ...
        % Check the bound and work out where we need to put the text
        [x_lims, y_lims, in_bounds] = getBounds(axes_handle, position);
        if (in_bounds)
            break
        elseif (~in_bounds && i == numel(axes_handle_all))
            return
        end
    end
